...
export class AdminStatsService {
  ...
  async getDashboardStats() {
    ...
    const previousTotalUsers = await this.prisma.user.count({ where: { createdAt: { lt: currentWeekStart } } });
    
    const totalCards = await this.prisma.card.count();
    const previousTotalCards = await this.prisma.card.count({ where: { createdAt: { lt: currentWeekStart } } });

    const totalTemplates = await this.prisma.template.count();
    const previousTotalTemplates = await this.prisma.template.count({ where: { createdAt: { lt: currentWeekStart } } });

    const publishedCards = await this.prisma.card.count({ where: { status: 'published' } });
    const previousPublishedCards = await this.prisma.card.count({ where: { status: 'published', createdAt: { lt: currentWeekStart } } });
    ...
    const getDailyData = async (model: any, filters = {}) => {
      const counts = await Promise.all(last7Days.map(async (day) => {
    ...
    const usersDaily = await getDailyData(this.prisma.user);
    const cardsDaily = await getDailyData(this.prisma.card);

    // Dữ liệu sparkline tượng trưng cho đẹp mắt (mock data)
    ...
    const topTemplates = await this.prisma.template.findMany({
      take: 5,
    ...
    return {
      stats,
      chartData,
      topTemplates: mappedTopTemplates,
      activity
    };
  }
}
