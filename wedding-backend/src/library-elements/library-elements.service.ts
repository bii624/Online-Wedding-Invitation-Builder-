...
import { CreateLibraryElementDto, UpdateLibraryElementDto, LibraryElementQueryDto } from './dto/library-element.dto';
import { CreateElementCategoryDto, UpdateElementCategoryDto } from './dto/element-category.dto';
...
export class LibraryElementsService {
            ...
            const uploadStream = this.cloudinary.uploader.upload_stream(
                {
                    ...
                    eager: [{ width: 300, height: 300, crop: 'pad', background: 'transparent', format: 'png' }],
                },
    ...
    async createCategory(dto: CreateElementCategoryDto, userId: string) {
        const existing = await this.prisma.elementCategory.findUnique({ where: { slug: dto.slug } });
    ...
    async getElements(query: LibraryElementQueryDto, adminMode = false) {
        const { search, categoryId, elementType, isPremium, status, page = 1, limit = 48 } = query;
    ...
    async uploadElementFile(id: string, file: Express.Multer.File, userId: string) {
        ...
        try {
            result = await this.uploadToCloudinary(file.buffer, `dearlove/library_elements`);
        } catch (error: any) {
            throw new BadRequestException(error.message || 'Lỗi upload file lên Cloudinary');
        }
    ...
    async updateElement(id: string, dto: UpdateLibraryElementDto) {
        ...
        return this.prisma.libraryElement.update({
            ...
            data: {
                ...
                ...(dto.requiredPlanId !== undefined && { requiredPlanId: dto.requiredPlanId ?? null }),
                ...(typeof dto.isRecolorable === 'boolean' && { isRecolorable: dto.isRecolorable }),
            },
        });
    }
    ...
    async toggleElementStatus(id: string) {
        ...
        if (newStatus === 'published' && !el.fileUrl) {
            throw new BadRequestException('Vui lòng upload file trước khi publish element');
        }
