...
export class CardsService {
  ...
  async approveWish(wishId: string, isApproved: boolean, userId: string) {
    ...
    if (wish.card.userId !== userId) {
      throw new ForbiddenException('Bạn không có quyền duyệt lời chúc này');
    }
  ...
  async deleteWish(wishId: string, userId: string) {
    ...
    if (!wish) throw new NotFoundException('Không tìm thấy lời chúc');
    if (wish.card.userId !== userId) {
      throw new ForbiddenException('Bạn không có quyền xóa lời chúc này');
    }
  ...
  async submitRsvp(cardId: string, dto: { guestName: string; phone?: string; attending: 'yes' | 'no' | 'maybe'; numAttendees?: number; note?: string }) {
    const card = await this.prisma.card.findUnique({ where: { id: cardId } });
    ...
    return this.prisma.rsvpResponse.create({
      data: {
        ...
        numAttendees: dto.numAttendees || 1,
        note: dto.note || null,
      },
    });
  }

  async submitWish(cardId: string, dto: { displayName: string; message: string; avatarUrl?: string }) {
    const card = await this.prisma.card.findUnique({ where: { id: cardId } });
    if (!card) throw new NotFoundException('Không tìm thấy thiệp cưới');
