...
export class CardsController {
  ...
  @ApiBearerAuth()
  @Get('wishes/all')
  @ApiOperation({ summary: 'Lấy tất cả lời chúc của các thiệp' })
  getAllWishes(@Req() req: AuthRequest) {
    return this.cardsService.getAllWishes(req.user.id);
  }

  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @Get('rsvp/all')
  @ApiOperation({ summary: 'Lấy tất cả phản hồi RSVP của các thiệp' })
  getAllRsvps(@Req() req: AuthRequest) {
    return this.cardsService.getAllRsvps(req.user.id);
  }

  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @Patch('wishes/:wishId/approve')
  @ApiOperation({ summary: 'Phê duyệt lời chúc' })
  approveWish(
    @Param('wishId', ParseUUIDPipe) wishId: string,
    @Body('isApproved') isApproved: boolean,
    @Req() req: AuthRequest,
  ) {
    return this.cardsService.approveWish(wishId, isApproved, req.user.id);
  }
  ...
  deleteWish(
    ...
    return this.cardsService.deleteWish(wishId, req.user.id);
  }

  @Post('public/:cardId/rsvp')
  @ApiOperation({ summary: 'Gửi phản hồi RSVP' })
  submitRsvp(
    @Param('cardId', ParseUUIDPipe) cardId: string,
    @Body() dto: { guestName: string; phone?: string; attending: 'yes' | 'no' | 'maybe'; numAttendees?: number; note?: string },
  ) {
    return this.cardsService.submitRsvp(cardId, dto);
  }

  @Post('public/:cardId/wishes')
  @ApiOperation({ summary: 'Gửi lời chúc mới' })
  submitWish(
    @Param('cardId', ParseUUIDPipe) cardId: string,
    @Body() dto: { displayName: string; message: string; avatarUrl?: string },
  ) {
    return this.cardsService.submitWish(cardId, dto);
  }
